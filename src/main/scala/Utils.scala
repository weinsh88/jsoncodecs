...
object Util:
  ...
  /** Parse a `json` document contained in a `String` value into a `Json` value
    ...
    *   a valid string
    * @return
    *   `Json` type. `None` in the case `s` is not a valid `Json` document
    */
  ...
  def render(json: Json): String = json match
    ...
    case JObject(vs) =>
      vs.map((k, v) => s"${fString(k)}:${render(v)}").mkString("{", ", ", "}")
  ...
  given Facade.SimpleFacade[Json] with
    ...
    def jnum(s: CharSequence, decIndex: Int, expIndex: Int): Json = JNumber(
      BigDecimal(s.toString)
    )
    def jstring(s: CharSequence): Json = JString(s.toString)
    def jobject(vs: Map[String, Json]): Json = JObject(vs)
